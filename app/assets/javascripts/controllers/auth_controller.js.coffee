...
HubHub.AuthController = Ember.Controller.extend
  currentUser: null
  isAuthenticated: Em.computed.notEmpty("currentUser.email")
  login: (route) ->
    me = @
    $.ajax
      url: HubHub.urls.login
      type: "POST"
      data:
        "user[email]": route.currentModel.email
        "user[password]": route.currentModel.password
      success: (data) ->
        log.log "Login Msg #{route.currentModel.email}"
        me.set "currentUser", route.currentModel
